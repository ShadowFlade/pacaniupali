    ...
    return games.map((game) => {
        const yourPlayer =
            (game.player &&
                game.player.length &&
                game.player.find(
                    (player) => player && player.user_id === currentUserID,
                )) ||
            null;
        const userWinner = game.player.find(
            (item) => item.id == game.winner.player_id,
        );
        console.log(userWinner, ' user winner');
        if (!game) return null;
        return (
            <table key={game.id} className="w-full rounded-lg bg-muted/50 p-2">
                <thead className="headers gap-4 text-left">
                    <th className="w-[200px]">Начало игры</th>
                    {game.winner && <th className="w-[200px]">Победитель</th>}
                    {game.player && (
                        <th className="w-[300px]">Количество участников</th>
                    )}
                    ...
                    {game.winner && (
                        <td>
                            <span className="font-medium">
                                <AvatarSmall
                                    picture={userWinner.user.picture}
                                    username={userWinner.user.username}
                                />
                            </span>
                        </td>
                    )}
                    {game.player && (
                        <td className="font-medium">{game.player.length}</td>
                    )}
                    {game.player && (
